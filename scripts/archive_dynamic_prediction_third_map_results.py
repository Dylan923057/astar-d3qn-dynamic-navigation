"""Archive the third-map validation failure and cross-map diagnostic."""
...
PHASE = "07_dynamic_prediction_third_map_validation"
...
def main() -> None:
    ...
    run_root = ROOT / "outputs" / "dynamic_prediction_third_map_v1" / "formal" / MAP_ID
    foundation_root = ROOT / "outputs" / "risk_handover_v1" / "formal" / MAP_ID
    summary_root = (
        ROOT
        / "outputs"
        / "dynamic_prediction_third_map_v1"
        / "analysis_validation"
        / MAP_ID
    )
    diagnostic_root = ROOT / "outputs" / "dynamic_prediction_cross_map_diagnostic_v1"
    ...
    if (
        ...
        or decision.get("generalizes")
        or decision.get("next_step") != "stop_and_diagnose"
    ):
        raise SystemExit("Expected a matched validation-only third-map stop decision.")
    if (
        not diagnostic.get("validation_only")
        or diagnostic.get("test_files_read")
        or diagnostic.get("robust_cross_map_generalization_supported")
    ):
        raise SystemExit("Expected a validation-only negative cross-map diagnostic.")
    ...
    copy_file(
        ROOT / "docs" / "DYNAMIC_PREDICTION_THIRD_MAP_V1.zh-CN.md",
        destination / "protocol" / "DYNAMIC_PREDICTION_THIRD_MAP_V1.zh-CN.md",
    )
    ...
    for name in ("diagnostic.json", "per_map_summary.csv", "per_seed_cross_map.csv", "report.md"):
        copy_file(diagnostic_root / name, destination / "analysis" / "cross_map_diagnostic" / name)
...
"""
    ...
    checklist_text = checklist_text.replace("分为六个阶段", "分为七个阶段")
