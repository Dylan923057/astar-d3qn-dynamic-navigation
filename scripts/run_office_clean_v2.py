"""Prepare and run the clean_v2 Office diagnostic pilot."""
...
MAIN_CONFIG = "configs/dynamic_office_clean_v2.yaml"
TIME_DECAY_CONFIG = "configs/dynamic_office_clean_time_decay_v2.yaml"
CA_ORACLE_CONFIG = "configs/dynamic_office_clean_ca_oracle_v2.yaml"
CORE_METHODS = ("uniform", "prefill", "persistent")
...
def _complete(method: Method, seed: int, smoke: bool) -> bool:
    ...
    if method.strategy != "uniform":
        ...
        demo_matches = (
            demo_path.exists()
            and isinstance(demo_dataset, dict)
            and demo_dataset.get("sha256")
            == hashlib.sha256(demo_path.read_bytes()).hexdigest()
        )
    gate_complete = smoke or (
        bool(curriculum.get("gate_enabled", False))
        and bool(curriculum.get("reached_final_stage", False))
        and bool(curriculum.get("transitions"))
    )
    return (
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="The default pilot runs only Uniform, Prefill, and Persistent.",
    )
    ...
    prepare = [
        sys.executable,
        str(ROOT / "scripts" / "prepare_office_clean_v1.py"),
        "--config",
        MAIN_CONFIG,
        "--audit-output",
        "outputs/office_clean_v2_dataset_design",
    ]
    ...
    print("[done] clean_v2 Office pilot completed.", flush=True)
